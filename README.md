# Netlab Tools (Proxmox Lab GUI + Netlabs Checker)
Flask webapp providing a lab VM portal similar to Azure Labs for self-service student VM access, now with an integrated Netlabs monitoring subsystem.

## Merged Architecture

- Host architecture remains the original `proxmox-lab-gui` Flask app (`app/routes`, `app/services`, `app/templates`).
- `netlabs-checker` behavior is integrated as a dedicated module:
  - Routes: `/netlabs`, `/netlabs/rooms/<room>`, `/api/heartbeat`, `/api/netlabs/heartbeat`, `/rdp/<hostname>.rdp`
  - Service layer: `app/services/netlabs_service.py`
  - Templates: `app/templates/netlabs/`
- Heartbeat state is persisted in `netlab_machines` (SQLAlchemy model `NetlabMachine`) with room inference and stale/offline status handling.
