...
class NetlabMachine(db.Model):
    """Netlabs heartbeat-backed machine state."""

    __tablename__ = 'netlab_machines'

    id = db.Column(db.Integer, primary_key=True)
    hostname = db.Column(db.String(120), unique=True, nullable=False, index=True)
    room = db.Column(db.String(64), nullable=False, default='Unknown', index=True)
    ...
    def __repr__(self):
        return f'<NetlabMachine {self.hostname} room={self.room}>'
